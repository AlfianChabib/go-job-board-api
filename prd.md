...
| PUT    | `/api/candidate/avatar`          | Memperbarui foto profil Candidate (`avatar_url`).                                              | `avatar_url: required, url`                                                      |
...
| GET    | `/api/recruiter/jobs`  | Melihat seluruh daftar lowongan milik recruiter yang sedang login, baik status `OPEN` maupun `CLOSED` (Recruiter Only).                              | Query: `page: omitempty,numeric,min=1`, `limit: omitempty,numeric,min=1,max=100`, `status: omitempty,oneof=OPEN CLOSED`                                                                                                                                                                                                                                                                                                |
| GET    | `/api/jobs/:id`        | Melihat detail lengkap lowongan beserta informasi perusahaan (Public).                                                                               | -                                                                                                                                                                                                                                                                                                                                                                                                                      |
| PUT    | `/api/jobs/:id`        | Memperbarui seluruh informasi lowongan kerja (Recruiter Only, wajib verifikasi kepemilikan/IDOR).                                                   | `title: required`, `description: required`, `requirements: omitempty`, `employment_type: required, oneof=FULL_TIME PART_TIME CONTRACT INTERNSHIP FREELANCE`, `work_mode: required, oneof=ON_SITE HYBRID REMOTE`, `location: required`, `min_salary: omitempty,numeric,min=0`, `max_salary: omitempty,numeric,gtefield=MinSalary`, `currency: omitempty,len=3`, `is_salary_negotiable: boolean`                         |
